...
#### POST `/api/cards/extract-words-from-photo/`
...
|------|------|-------------|----------|
| `image` | File | Да | Фото текста (JPEG/PNG, макс 10 МБ) |
| `target_lang` | string | Да | Код изучаемого языка (`de`, `en`, `fr`, ...) |
| `source_lang` | string | Да | Код родного языка (`ru`, `en`, ...) |
...
```json
{
  "words": ["Haus", "Schule", "lernen", "Freund"]
}
```

**Ошибки:**
- `400` — отсутствует файл, неподдерживаемый формат, файл слишком большой, не указаны языки
- `401` — не авторизован
